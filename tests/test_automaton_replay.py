...
def _env(n: int = 200, alphabet: int = 3, seed: int = 0) -> np.ndarray:
    return np.random.default_rng(seed).integers(0, alphabet, size=n).astype(np.int64)


def test_self_generated_replay_shifts_trajectory_by_one_orbit_step() -> None:
    aut = SimpleAutomaton.from_self_generated_rules(n_states=4, env=_env(n=200, seed=1), seed=2)
...
def test_self_generated_replay_stays_inside_alphabet() -> None:
    aut = SimpleAutomaton.from_self_generated_rules(n_states=4, env=_env(n=200, seed=3), seed=4)
...
def test_replay_rejects_out_of_range_t_star() -> None:
    aut = SimpleAutomaton.from_self_generated_rules(n_states=3, env=_env(n=50, seed=7), seed=8)
...
def test_replay_rejects_horizon_overrun() -> None:
    aut = SimpleAutomaton.from_self_generated_rules(n_states=3, env=_env(n=50, seed=9), seed=10)
